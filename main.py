...
# --- MAN HINH BAN HANG (CẢI THIỆN TÌM KIẾM BẰNG TEXT) ---
            ...
            if not filtered_df.empty:
                ...
                if selected_str:
                    selected_item = filtered_df[
                        (filtered_df['TenSanPham'] + " | Mã: " + filtered_df['MaSanPham'] + " | Tồn: " + filtered_df['SoLuong'].astype(int).astype(str) + " " + filtered_df['DonVi']) == selected_str
                    ].iloc[0]
...
# --- MAN HINH BAO CAO (ĐÃ FIX BIỂU ĐỒ, THÊM NHIỀU TÍNH NĂNG) ---
def render_reports(df_inv):
    ...
    with t2:
            ...
            if not df_today.empty:
                ...
                for idx, row in df_today.iterrows():
                    with st.container(border=True):
                        ...
                        if row['SoLuong'] > 0:  # Chỉ cho hoàn trả nếu là bán (số lượng dương)
        ...
        # Biểu đồ doanh thu & lợi nhuận tháng này
    ...
    if not st.session_state['is_logged_in']:
        ...
        with st.sidebar:
            ...
            if st.button("Đăng Xuất"):
                st.session_state['is_logged_in'] = False
                st.rerun()
            st.caption("Minh Châu 24h v3.0 - Đã nâng cấp")
